...
local function build_dungeon_tile_floor_and_roof_and_walls(pos, floor_type, roof_type, wall_type_1, wall_type_2, pillar_type, x_plus, x_minus, z_plus, z_minus,
	...
	if has_room then
		made_room = make_room(pos, false, false, floor_type, wall_type_1, wall_type_2, roof_type, pillar_type, x_plus, x_minus, z_plus, z_minus, room_style, rooms_data, dungeon_data)
	end
...
local function add_artificial_caves(pos, width, height_in_blocks, wanted_cave_percentage, top_staircase_height, bubble_cave_data, dungeon_data)
	...
	while true do
		...
		if minetest.get_node(bubble_pos).name == "air" and bubble_pos.y < -30 and minetest.get_node_light(bubble_pos, 0.5) == 0 then
			...
			table.shuffle(nature_blocks_to_grow_directly)
			for _, pos2 in ipairs(nature_blocks_to_grow_directly) do
				make_nature(pos2, dungeon_data)
			end
		else
	...
	while true do
		...
		if not minetest.find_node_near(bubble_pos, bubble_radius+1, {"air", "group:liquid"}) then
			...
			if rand_num < 0.25 then
				...
				cave_floor_flattened = math.random() < 0.25
			elseif rand_num < 0.6 then
				material = "default:water_source"
				cave_floor_flattened = math.random() < 0.4
			else
				material = "air"
				cave_floor_flattened = math.random() < 0.65
			end
			...
			local exact_fill_height_relative_to_pos = bubble_pos.y - bubble_radius* 2 -- <-- set this to below the bubble ground by default
			if pegel then
				exact_fill_height_relative_to_pos = -bubble_radius + 2 * bubble_radius * pegel
			end
			exact_fill_height_relative_to_pos = pegel and -bubble_radius + 2 * bubble_radius * pegel
	...
	-- print("finished generating caves !")
end
...
local function make_dungeon_level(pos, width, floor_type, wall_type_1, wall_type_2, roof_type, pillar_type, dungeon_deph, staircase_height, pillar_height, rim_sealed,
	...
	for x = 1, width do
		for z = 1, width do
			table.insert(tile_coords, {x=x, z=z})
		end
	end
	table.shuffle(tile_coords)
	for _, coord in ipairs(tile_coords) do
		local x = coord.x
		local z = coord.z
...
local function make_dungeon(pos, width, floor_type, wall_type_1, wall_type_2, roof_type, pillar_type, bridge_type, dungeon_deph, rim_sealed, dungeon_levels,
	...
	add_artificial_caves(pos, width, dungeon_top_deph + dungeon_bottom_deph + (dungeon_levels - 1) * dungeon_deph, cave_percentage/100, dungeon_top_deph, this_dungeon.bubble_caves, this_dungeon)
